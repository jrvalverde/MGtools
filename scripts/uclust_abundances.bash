...
function usage {
    ...
    This will produce a number of files (abund, richness) and graphic
    charts in PNG format (abund.png, labund.png, llabund.png, preston,png)
    in the same directory as the input file is located. Note: the richness
    file contains the OTU sizes, and may be used for curve-fitting.
...
notus=`cut -d '	' -f2 $readmap_uc | uniq | wc -l`
...
cat richness | sed -e 's/^ *//g' | sed -e 's/ /	/g' | sort -t'	' -k 2 -g > richness.tab
cat abund | sed -e 's/^ *//g' | sed -e 's/ /	/g' | sort -t'	' -k 2 -g > abund.tab

# richness contains pairs   (N-indivs-of	OTU-number)
# abund contains pairs      (NOTUS-with	    	N-indivs-per-OTU)
