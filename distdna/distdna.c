 ...
 * 
 *
 * $Id: distdna.c,v 1.24 2011/01/18 10:22:54 jr Exp jr $
 *
 * $Log: distdna.c,v $
 * Revision 1.24  2011/01/18 10:22:54  jr
 * Avoid spaces in phylip names [j]
 *
 * Revision 1.23  2011/01/18 09:57:17  root
 * Added gap check function to account for different gap characters [j]
 *
...
int main(int argc, char **argv)
{
    ...
    do {
	i = getopt_long(argc, argv, "i:o:g:a:?hl1pxs", opts, &j);
	switch (i) {
	case '?':
	    usage(argv[0]);
	    break;
	...
	case 'a':
	    ...
	    break;
	case 's':
	    oflags |= (LOWER_TRIANGULAR | UNITY_DISTANCE | PHYLIP_SIMPLE | PHYLIP);
	    break;
	case 'l':
...
#define CHUNKSIZE	1024*8
    int i, insize, memsize, nseqs, seqlen, newlen;
    char **seqs, **seqnames, *s, line[BUFSIZ+1];
 ...
 *	Parallelization:
 *
 *	The obvious strategy is to compute as many distances as CPUs
 *	are available, wait for all to complete (should be more or
 *	less the same as they are all same length) and then output
 *	distances in order before proceeding to next batch. Whether
 *	this is best done as threads or using OpenMP needs to be
 *	decided.
 *
 *	This should work OK on multiprocessors where communication
 *	cost is low. If communications are expensive, like e.g. on
 *	and MPI cluster, then the approach would be slightly
 *	different: split the total number of distances to compute in
 *	as many blocks as computers there are. Then send each block
 *	of data to its target computer and wait for all to complete
 *	in sucession writing orderly as data becomes available.
 *	This approach calls for a more complex pattern, both for
 *	determining the block contents and their associated data.
 *
 *	The third approach would be a combination of both for clusters
 *	of multiprocessors: split the problem in blocks and then process
 *	each block in CPU sets at the target processor.
 *
 */
	...
	for (j = 0; j < nseqs; j++) {
	    if (oflags & LOWER_TRIANGULAR) {
	      if (j > i) {
	        if (oflags & PHYLIP_SIMPLE) continue;
		fprintf(outfile, "\t");
		DBG(fflush(outfile));
		continue;
	      }
...
/*
		...
		for (i = 0; i<MAXTASKS; i++) {
		    while (task(tid) not done); / * wait for completion * /
		    output dist[tid];
		}
		tid = 0;
	      }
	     ...
	     enum correction_t correction, int oflags)
{
    ...
    if (oflags & PHYLIP_SIMPLE) {
    	/* PHYLIP simplified format as defined in MOTHUR: */
	/* nseqs
	   S1
	   S2	d(1:2) ... */
        fprintf(outfile, "%d", nseqs);
    } else if (oflags & PHYLIP) {
        for (i = 0; i < nseqs; i++) {
	    char snam[11];
	    int count;
	    for (count = 0; count < 10; count++)
...
double nt_ambig_match(char n1, char n2)
{
	...
	V = A|C|G,
	N = A|C|G|T
    };	/* IUPAC codes in binary form for quick comparison */
